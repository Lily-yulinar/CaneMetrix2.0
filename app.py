...
# --- FUNGSI KONEKSI GOOGLE SHEETS (TAMBAHAN UNTUK EXCEL) ---
def koneksi_ke_excel():
    scopes = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
    try:
        # Mengambil info dari Secrets Streamlit Cloud
        secret_info = st.secrets["gcp_service_account"]
        creds = Credentials.from_service_account_info(secret_info, scopes=scopes)
        client = gspread.authorize(creds)
        # Buka file KKKB_250711 dan sheet INPUT
        return client.open("KKKB_250711").worksheet("INPUT")
...
if st.session_state.page == 'dashboard':
    ...
    if st.session_state.analisa_type == 'tetes':
        ...
        with st.container():
            ...
            cx, cy = st.columns(2)
            with cx:
                jam_pilih = st.selectbox("Analisa Jam", [6, 14, 22], index=0)
                ...
                # --- TOMBOL SIMPAN EXCEL (UPDATE DISINI) ---
                if st.button("🚀 SIMPAN KE EXCEL", type="primary", use_container_width=True):
                    sheet = koneksi_ke_excel()
                    if sheet:
                        try:
                            waktu_log = datetime.datetime.now(pytz.timezone('Asia/Jakarta')).strftime("%Y-%m-%d %H:%M:%S")
                            sheet.append_row([waktu_log, jam_pilih, bx_in, sh_in, pol_baca, brix_akhir, pol_akhir, hk])

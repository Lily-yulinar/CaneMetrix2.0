...
# State untuk Rekap Harian (Logika AVERAGEIF Excel)
# Menyimpan list data per jam agar bisa dirata-rata otomatis
if 'rekap_brix' not in st.session_state:
    st.session_state.rekap_brix = {
        "NPP": [], "Gilingan 2": [], "Gilingan 3": [], "Gilingan 4": []
    }
...
data_tsai = {15.0: 336.00, 16.0: 316.00, 17.0: 298.00, 18.0: 282.00, 19.0: 267.00, 20.0: 254.50, 21.0: 242.90, 22.5: 223.60, 23.0: 222.20, 24.0: 213.30, 25.0: 204.80, 26.0: 197.40, 27.0: 190.40, 28.0: 183.70, 29.0: 177.60, 30.0: 171.70, 31.0: 166.30, 32.0: 161.20, 33.0: 156.60, 34.0: 152.20, 35.0: 147.90, 36.0: 143.90, 37.0: 140.20, 37.7: 136.67}
...
if st.session_state.page == 'dashboard':
    ...
    with c3:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>📊</h1></div>", unsafe_allow_html=True)
        if st.button("COCKPIT", key="dash_kpi", use_container_width=True): st.session_state.page = 'kpi_monitoring'; st.rerun()
    with c4:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>📈</h1></div>", unsafe_allow_html=True)
        if st.button("KURVA BRIX", key="dash_kurva", use_container_width=True): st.session_state.page = 'kurva_brix'; st.rerun()
    ...
    for i, (kpi, val) in enumerate(st.session_state.kpi_data.items()):
        ...
        with cols[i]:
            st.markdown(f"""
                <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 15px; border-top: 5px solid {warna}; text-align: center;">
                    <p style="color: white; font-size: 12px; margin:0;">{kpi}</p>
                    <h3 style="color: {warna}; font-family: Orbitron; margin: 5px 0;">{val}%</h3>
    ...
    if st.session_state.kpi_data["ME"] < STANDAR["ME"]:
        st.warning("⚠️ **ME RENDAH**: Optimalkan tekanan hidrolik gilingan & cek kehalusan ampas.")
    if st.session_state.kpi_data["OR"] < STANDAR["OR"]:
        st.error("🚨 **OVERALL RECOVERY DROP**: Segera cek losses di ampas, blotong, dan tetes!")
    st.markdown('</div>', unsafe_allow_html=True)
    
    st.markdown("<br>", unsafe_allow_html=True)
    st.subheader("📉 Tren Recovery Shift Berjalan")
    trend_df = pd.DataFrame({
        "Jam": ["06:00", "08:00", "10:00", "12:00", "14:00"],
        "OR Aktual": [85.0, 85.5, 85.1, 84.8, 85.1],
    ...
    st.markdown("<h2 style='text-align:center; color:#26c4b9; font-family:Orbitron;'>📈 KURVA BRIX GILINGAN (RATA-RATA HARIAN)</h2>", unsafe_allow_html=True)
    
    # Logika Hitung Rata-rata dari Rekap (Logika Excel Averageif > 0)
    def get_avg(prefix):
        data = st.session_state.rekap_brix.get(prefix, [])
        valid = [v for v in data if v > 0]
        return sum(valid) / len(valid) if valid else 0.0

    nyata = [get_avg("NPP"), get_avg("Gilingan 2"), get_avg("Gilingan 3"), get_avg("Gilingan 4")]
    ...
    st.info(f"📊 Menampilkan rata-rata dari seluruh data yang telah di-input hari ini.")
    
    lamda = imb / sabut if sabut > 0 else 0
    
    # Perhitungan Teoritis Bni (Sesuai Rumus Pabrik)
    teoritis = [nyata[0]]
    if nyata[0] > 0 and lamda > 0:
        eg = 4 # jumlah gilingan
        for gi in range(2, 5):
            bni = nyata[0] * ((lamda**(eg+1-gi) - 1) / (lamda**eg - 1))
            teoritis.append(round(bni, 2))
    else:
        teoritis = [nyata[0], 0, 0, 0]
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=["G1 (NPP)", "G2", "G3", "G4"], y=nyata, name='Realisasi (Avg)', line=dict(color='#ff4b4b', width=4), mode='lines+markers'))
    fig.add_trace(go.Scatter(x=["G1 (NPP)", "G2", "G3", "G4"], y=teoritis, name='Teoritis', line=dict(color='#26c4b9', width=4), mode='lines+markers'))
    ...
    if st.button("🗑️ RESET REKAP HARIAN", type="secondary"):
        st.session_state.rekap_brix = {"NPP": [], "Gilingan 2": [], "Gilingan 3": [], "Gilingan 4": []}
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
    if st.button("🔙 KEMBALI KE DASHBOARD", use_container_width=True): st.session_state.page = 'dashboard'; st.rerun()
    ...
    # Pilih Jam (Hanya label, data masuk ke list rata-rata)
    list_jam_ops = [f"{(i % 24):02d}:00" for i in range(6, 30)]
    jam_aktif = st.selectbox("📍 Masukkan Data Untuk Jam:", list_jam_ops)
    ...
    def render_logic_brix_pol(prefix):
        ...
        with c_input:
            ...
            bx_in = st.number_input("Brix Baca", value=0.0, key=f"bx_{prefix}_{jam_aktif}", format="%.2f")
            sh_in = st.number_input("Suhu (°C)", value=28.0, key=f"sh_{prefix}_{jam_aktif}", format="%.1f")
            pol_in = st.number_input("Pol Baca", value=0.0, key=f"pol_{prefix}_{jam_aktif}", format="%.2f")
            ...
            if st.button(f"🚀 SIMPAN DATA {prefix} JAM {jam_aktif}", key=f"btn_{prefix}"): 
                if prefix in st.session_state.rekap_brix:
                    st.session_state.rekap_brix[prefix].append(bx_fix)
                st.toast(f"Data {prefix} Jam {jam_aktif} Direkap!")
        with c_hasil:
    ...
    def render_input_lengkap(prefix, list_opsi, force_direct=False):
        if not force_direct:
            sub = st.selectbox(f"Pilih Analisa {prefix}", list_opsi, key=f"sel_{prefix}_{jam_aktif}")
        else:
            sub = "(Brix, Pol, dan HK)"
    ...
    menu_neraca = ["(Brix, Pol, dan HK)", "(Gula Reduksi)", "(Kadar Posfat)", "(Dextran)"]
    ...
    # Analisa lab tetap menggunakan logika yang lama namun pastikan variabel jam tidak error

...
# --- 2. CSS (UPDATE KOTAK LOGO) ---
st.markdown(f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Michroma&family=Orbitron:wght@400;700&family=Poppins:wght@300;400;700&display=swap');
    ...
    /* UPDATE: KOTAK LOGO KIRI ATAS LEBIH PANJANG */
    .partner-box {{ 
        background: white; 
        padding: 12px 60px; /* Padding kanan-kiri ditambah biar panjang */
        border-radius: 12px; 
        display: inline-flex; 
        align-items: center; 
        gap: 55px; /* Jarak antar logo diperbesar */
        box-shadow: 0 4px 15px rgba(0,0,0,0.3);
        min-width: 450px; /* Lebar minimal kotak putih */
    }}
    ...
    .menu-card-container:hover {{
        ...
        box-shadow: 0 0 30px rgba(38, 196, 185, 0.4); transform: translateY(-5px);
    }}
...
if st.session_state.page == 'dashboard':
    # HEADER (LOGIKA LAYOUT DIPERBAIKI)
    c_top1, c_top2 = st.columns([3, 1]) # Kolom pertama lebih lebar untuk mengakomodasi kotak putih
    with c_top1:
        st.markdown(f'''
    ...
    with c2:
        ...
        st.markdown(f"""
            <div style="background: rgba(38, 196, 185, 0.2); padding: 30px; border-radius: 20px; border: 2px solid #26c4b9; text-align: center;">
                ...
                <h1 style="color:#26c4b9; font-family:Michroma; font-size:50px; margin:10px 0;">{bx_akhir:.3f}</h1>
            </div>
        """, unsafe_allow_html=True)

...
STANDAR = {
    "OR": 86.0, "ME": 96.2, "BHR": 90.0, "HPB TOTAL": 78.0, "PSHK": 75.0
}

# Dummy Data untuk KPI (Nanti dihubungkan ke rumus asli)
if 'kpi_data' not in st.session_state:
    st.session_state.kpi_data = {
        "OR": 85.2, "ME": 96.5, "BHR": 88.0, "HPB TOTAL": 76.5, "PSHK": 73.0
    }
...
if st.session_state.page == 'dashboard':
    ...
    with c3:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>📊</h1></div>", unsafe_allow_html=True)
        if st.button("MANAGEMENT COCKPIT", key="dash_kpi"): st.session_state.page = 'kpi_monitoring'; st.rerun()
    ...
    # Render KPI Cards
    cols = st.columns(len(st.session_state.kpi_data))
    for i, (kpi, val) in enumerate(st.session_state.kpi_data.items()):
        target = STANDAR.get(kpi, 0)
        warna = "#ff4b4b" if val < target else "#26c4b9"
        with cols[i]:
            st.markdown(f"""<div style="background:rgba(255,255,255,0.05); padding:20px; border-radius:15px; border-top:5px solid {warna}; text-align:center;"><p style="color:white; font-size:14px; margin:0;">{kpi}</p><h2 style="color:{warna}; font-family:Orbitron; margin:10px 0;">{val}%</h2><p style="color:#888; font-size:12px;">Std: {target}%</p></div>""", unsafe_allow_html=True)
    ...
    st.subheader("🕵️ Analisis & Navigasi Tindakan")
    for kpi, val in st.session_state.kpi_data.items():
        target = STANDAR[kpi]
        if val < target:
            if kpi == "ME": st.warning(f"⚠️ **ME Rendah ({val}%)**: Optimalkan imbibisi dan cek keausan roll gilingan.")
            elif kpi == "OR": st.error(f"🚨 **OVERALL RECOVERY LOW**: Audit losses di ampas dan tetes segera!")
            else: st.warning(f"⚠️ **{kpi} di bawah standar**: Cek kembali operasional terkait.")
        else: st.success(f"✅ **{kpi} Aman**: Performa sudah sesuai standar normatif.")
    st.markdown('</div>', unsafe_allow_html=True)

    # Grafik Tren
    st.markdown("<br>", unsafe_allow_html=True)
    st.subheader("📉 Tren Overall Recovery (OR) Shift Berjalan")
    chart_data = pd.DataFrame({"Jam": ["06:00", "10:00", "14:00", "18:00", "22:00", "02:00"], "OR Aktual": [84.5, 85.2, 85.0, 85.8, 86.1, 85.9], "Target": [86.0]*6}).set_index("Jam")
    ...
    with r1c1:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>🚜</h1></div>", unsafe_allow_html=True)
        if st.button("STASIUN GILINGAN"): st.session_state.page = 'input_gilingan'; st.rerun()
    with r1c2:
        ...
        if st.button("STASIUN PEMURNIAN"): st.toast("Coming Soon")
    with r1c3:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>🔥</h1></div>", unsafe_allow_html=True)
        if st.button("STASIUN PENGUAPAN"): st.toast("Coming Soon")
    ...
    tabs = st.tabs(["NPP", "Gilingan 2", "Gilingan 3", "Gilingan 4", "Nira Mentah", "Ampas", "Imbibisi"])
    ...
    def render_input_lengkap(prefix, list_opsi):
        ...
        c_in, c_res = st.columns([1, 2.2])
        with c_in:
            bx_in = st.number_input("Brix Baca", value=0.0, key=f"bx_{prefix}")
            sh_in = st.number_input("Suhu", value=28.0, key=f"sh_{prefix}")
            pol_in = st.number_input("Pol Baca", value=0.0, key=f"pl_{prefix}")
            kor = hitung_interpolasi(sh_in, data_koreksi); bj = hitung_interpolasi(bx_in, data_bj)
            bx_f = (bx_in + kor) if bx_in > 0 else 0
            pol_f = (0.286 * pol_in) / bj if bj > 0 else 0
            hk_f = (pol_f / bx_f * 100) if bx_f > 0 else 0
            if st.button(f"💾 SIMPAN {prefix}"): st.toast("Data Disimpan!")
        with c_res: tampilkan_kartu_hasil(bx_f, pol_f, hk_f)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tabs[4]: render_input_lengkap("Nira Mentah", menu_neraca + ["(TSAS)"])
    ...
    st.markdown(f"<h2 style='text-align:center; color:#26c4b9; font-family:Orbitron;'>🔬 ANALISA {st.session_state.analisa_type.upper()}</h2>", unsafe_allow_html=True)
    st.markdown('<div class="hero-container" style="display:block; padding:30px;">', unsafe_allow_html=True)
    cx, cy = st.columns([1, 2.2])
    with cx:
        bx = st.number_input("Brix", value=8.8); sh = st.number_input("Suhu", value=28.0); pol = st.number_input("Pol", value=11.0)
        kor = hitung_interpolasi(sh, data_koreksi); bj = hitung_interpolasi(bx, data_bj)
        res_bx = (bx + kor) * 10; res_pl = (0.286 * pol) / bj * 10; res_hk = (res_pl / res_bx * 100) if res_bx > 0 else 0
    with cy: tampilkan_kartu_hasil(res_bx, res_pl, res_hk)
    st.markdown('</div>', unsafe_allow_html=True)

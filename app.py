...
st.markdown(f"""
    ...
    .title-text {{
        ...
        text-shadow: 0 0 10px #fff, 0 0 42px #26c4b9;
    }}
    .menu-card {{
    ...
    /* Style khusus agar tombol di dalam card transparan */
    div.stButton > button {{
...
if st.session_state.page == 'dashboard':
    ...
    # Hero
    st.markdown(f'<div class="hero-container"><img src="data:image/png;base64,{logo_cane}" style="height:120px;"><h1 class="title-text">CANE METRIX</h1><p style="color:#26c4b9; font-weight:700; letter-spacing:5px;">ACCELERATING QA PERFORMANCE</p></div>', unsafe_allow_html=True)
        ...
        for j in range(3):
            if i + j < len(items):
                ...
                with cols[j]:
                    ...
                    # Jika Klik HITUNG, pindah ke halaman analisa_tetes
                    if st.button(f"{icon}\n\n{text.upper()}", key=f"btn_{text}"):
                        if text == "Hitung":
        ...
        with col2:
            ...
            st.markdown(f"""
                <div style="background: rgba(38, 196, 185, 0.2); padding: 20px; border-radius: 15px; border: 2px solid #26c4b9; text-align: center;">
                    <p style="margin:0; font-family:Poppins; opacity:0.8; color:white;">Brix Pengenceran (x10): <b>{bx_x10:.2f}</b></p>
                    <hr style="border-color: rgba(255,255,255,0.1);">
                    <h4 style="margin:0; font-family:Poppins; color:white;">% BRIX AKHIR</h4>
                    <h1 style="margin:0; color:#26c4b9; font-family:Orbitron; font-size:50px; text-shadow: 0 0 10px #26c4b9;">{bx_akhir:.3f}</h1>
                </div>
            """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

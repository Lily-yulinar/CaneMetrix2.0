...
# --- FUNGSI KIRIM EXCEL (BARU) ---
def kirim_ke_excel(jam, brix=None, pol=None, tsai=None, od=None):
    try:
        info_kred = dict(st.secrets["gcp_service_account"])
        info_kred["private_key"] = info_kred["private_key"].replace("\\n", "\n")
        scope = ["https://www.googleapis.com/auth/spreadsheets"]
        creds = Credentials.from_service_account_info(info_kred, scopes=scope)
        client = gspread.authorize(creds)
        sheet = client.open_by_key("1yQ2DbMy0ip_du1gqJ16jWwaWK8Psv6AB").worksheet("INPUT")

        # Rumus Baris: Jam 06:00 = Baris 270
        baris = 270 + (jam - 6) if jam >= 6 else 270 + (jam + 18)
...
if st.session_state.page == 'dashboard':
    ...
    with m3:
        ...
        if st.button("ANALISA TSAI TETES", key="sel_tsai", use_container_width=True): st.session_state.page = 'analisa_lab'; st.session_state.analisa_type = 'tsai'; st.rerun()
    with m4:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>💎</h1></div>", unsafe_allow_html=True)
    ...
    # --- PILIH JAM UNTUK SEMUA ANALISA ---
    st.markdown("<div style='background:rgba(255,255,255,0.1); padding:20px; border-radius:15px; margin-bottom:20px;'>", unsafe_allow_html=True)
    jam_analisa = st.selectbox("🕒 JAM ANALISA (SESUAI BARIS EXCEL)", list(range(24)), index=6)
    ...
    if st.session_state.analisa_type == 'tetes':
        ...
        with cx:
            ...
            if st.button("🚀 KIRIM BRIX & POL KE EXCEL", use_container_width=True):
                if kirim_ke_excel(jam_analisa, brix=brix_akhir, pol=pol_akhir):
                    st.success(f"✅ Data Jam {jam_analisa}:00 Berhasil dikirim ke Excel!")
        with cy:
        ...
        with cx:
            ...
            if st.button("🚀 KIRIM OD KE EXCEL", use_container_width=True):
                if kirim_ke_excel(jam_analisa, od=od_res):
                    st.success(f"✅ Data OD Jam {jam_analisa}:00 Berhasil dikirim!")
        with cy:
        ...
        with cx:
            ...
            if st.button("🚀 KIRIM TSAI KE EXCEL", use_container_width=True):
                if kirim_ke_excel(jam_analisa, tsai=tsai_final):
                    st.success(f"✅ Data TSAI Jam {jam_analisa}:00 Berhasil dikirim!")
        with cy:
        ...
        with cx:
            ...
            st.write("Icumsa biasanya tidak masuk ke baris rutin Tetes, simpan manual atau hubungi admin.")
        with cy:

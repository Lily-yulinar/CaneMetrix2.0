...
# Data BJ (Hanya sampel singkat, sistem akan interpolasi otomatis)
data_bj = {
...
if st.session_state.page == 'dashboard':
    ...
    with c1:
        ...
        if st.button("INPUT DATA", key="dash_input"): st.toast("Segera Hadir")
    with c2:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>🧮</h1></div>", unsafe_allow_html=True)
        if st.button("HITUNG ANALISA", key="dash_hitung"):
            st.session_state.page = 'pilih_analisa'; st.rerun()
    with c3:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>📅</h1></div>", unsafe_allow_html=True)
        if st.button("DATABASE HARIAN", key="dash_db"): st.toast("Segera Hadir")
    ...
    with m2:
        st.markdown("<div style='text-align:center; margin-bottom:-55px; position:relative; z-index:10; pointer-events:none;'><h1>🔬</h1></div>", unsafe_allow_html=True)
        # NAMA SUDAH DIUPDATE: OPTICAL DENSITY TETES
        if st.button("OPTICAL DENSITY TETES", key="sel_od", use_container_width=True):
    ...
    if st.session_state.analisa_type == 'tetes':
        ...
        # ... (Logika Analisa Tetes tetap sama) ...
        # [Logika Brix, Pol, HK seperti sebelumnya]
